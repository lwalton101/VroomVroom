...
class Controller(Component):
    ...
    def Update(self) -> None:
        ...
        if InputManager.IsKeyPressed(pygame.K_w):
            self.gameObject.pos = self.gameObject.pos[0], self.gameObject.pos[1] + (self.speed * Time.dt)
        if InputManager.IsKeyPressed(pygame.K_a):
            self.gameObject.pos = self.gameObject.pos[0] - (self.speed * Time.dt), self.gameObject.pos[1]
